# 1. `toolbox.py`
> Skrip ini dijalankan untuk menguji tool yang sudah kita definisikan di `tools.yaml`
```python
import os
import asyncio
...
async def main():
    async with ToolboxClient(server_url,client_headers=headers) as toolbox:
        tool = await toolbox.load_tool("tool_name")
        result = await tool(parameter_name="Insert parameter here")
        print(result)
...
---
# 1. `agent.py`
> file ini adalah skrip utama framework AI Agen yang akan kita gunakan.
```python
import os
...
---
# 2. A2A
> Dibawah ini adalah skrip untuk menjalankan agen kita sebagai A2A
```python
...
```
> Menambahkan agent card directory
```python
...
# 3. Email-Based Access Control Middleware
```python
import requests
